...
class FirebaseService {
  ...
  async submitRating(driverUid: string, rideId: string, score: number, comment: string, userId: string): Promise<void> {
    ...
    const ratingsRef = ref(database, `ratings/${driverUid}`);
    const snapshot = await get(ratingsRef);

    if (snapshot.exists()) {
      const ratings = snapshot.val();
      const ratingValues = Object.values(ratings) as Array<{ score: number }>;
      const avg = ratingValues.reduce((sum, r) => sum + r.score, 0) / ratingValues.length;

      await update(ref(database, `drivers/${driverUid}`), {
